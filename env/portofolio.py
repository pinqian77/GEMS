...
from utils.util import date_to_index, index_to_date
...
class DataGeneratorMain(object):
    ...
    def __init__(self, history,
        ...
        self.oracle_window = int(window_length/2)
    ...
    def _step(self):
        ...
        s_obs = self.data[:, self.step:self.step + self.window_length, :].copy()
        o_obs = self.data[:, self.step + self.oracle_window:self.step + self.oracle_window + self.window_length, :].copy()
        ...
        return s_obs, o_obs, done, ground_truth_obs
    ...
    def reset(self):
        ...
        if self.valid_env:
            ...
            self.idx = np.random.randint(
                low=self.window_length, high=self._data.shape[1] - self.steps - self.oracle_window)
        ...
        data = self._data[:, self.idx - self.window_length:self.idx + self.steps + self.oracle_window, :]
        ...
        return self.data[:, self.step:self.step + self.window_length, :].copy(), \
               self.data[:, self.step + self.oracle_window:self.step + self.oracle_window + self.window_length, :].copy(), \
...
class PortfolioSim(object):
    ...
    def _step(self, w1_o, v1, v2):
        ...
        w1_c = (v1 * w1_o) / (np.dot(v1, w1_o) + eps)
...
class PortfolioEnvMain(gym.Env):
    """
    ...
    def __init__(self,
        ...
        self.src = DataGeneratorMain(history, abbreviation, timestamp,steps=steps,
    ...
    def _step(self, action):
        ...
        observation, o_observation, done1, ground_truth_obs = self.src._step()

        # concatenate observation with ones
        cash_observation = np.ones((1, self.window_length, observation.shape[2]))
        o_cash_observation = np.ones((1, self.window_length, o_observation.shape[2]))

        observation = np.concatenate((cash_observation, observation), axis=0)
        o_observation = np.concatenate((o_cash_observation, o_observation), axis=0)
        ...
        return observation, o_observation, onestep_r, done1 or done2, info, weights
    ...
    def _reset(self):
        ...
        self.sim.reset()
        pre_w = self.sim.w0_c
        observation, o_observation, ground_truth_obs = self.src.reset()
        self.timestamp_all = self.src.timestamp_all

        cash_observation = np.ones((1, self.window_length, observation.shape[2]))
        observation = np.concatenate((cash_observation, observation), axis=0)
        o_observation = np.concatenate((cash_observation, o_observation), axis=0)
        cash_ground_truth = np.ones((1, 1, ground_truth_obs.shape[2]))
        ground_truth_obs = np.concatenate((cash_ground_truth, ground_truth_obs), axis=0)
        info = {}
        info['next_obs'] = ground_truth_obs

        return observation, o_observation, info
